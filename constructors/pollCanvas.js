...
module.exports = class PollCanvas{
    ...
    getCanvasHeight({closed=false, title=false, options=false, footer=false, image=true}={}) {
        ...
        this.titleHeight = splitCanvasString(ctx, this.title)
                         .filter(Boolean).length*40 + 60
    ...
    setTitle() {
        ...
        if (textwidth > 800) {
            ...
            stringArray.forEach((string, i) => {
                ...
                this.ctx.fillText(string, (this.canvas.width/2) - (stringWidth / 2), 60 + this.closed + (i*40))
            })
        } else {
            this.ctx.fillText(this.title, (this.canvas.width/2) - (textwidth / 2), 60 + this.closed)
        }
    ...
    setImage() {        
        ...
        this.ctx.drawImage(this.image.image, center, (this.closed + this.titleHeight), this.image.width, this.image.height)
    }
    ...
    setFooter() {
        ...
        this.ctx.fillText(`PollID: ${this.id}`, 75, this.footerContentHeight + this.closed + this.urlimage)
        this.ctx.fillText(date, footerDateWidth, this.footerContentHeight + this.closed + this.urlimage)
    }
}